...
let slider_about = new Swiper('.slider-main', {
...
window.onload = function () {
	...
	const posts = document.querySelector('.inst-gallery__body')
	if (posts && !isMobile.any()) {
		const postsItems = document.querySelector('.inst-gallery__items')
		const postsItem = document.querySelectorAll('.inst-gallery__item')

		// Скорость анимации

		const speed = posts.dataset.speed;
		...
		function setMouseGalleryStyle() {
			let postsItemsWidth = 0;
			postsItem.forEach(element => {
				postsItemsWidth += element.offsetWidth;
			});
			

			const postsDifferent = postsItemsWidth/2 - posts.offsetWidth;
			const distX = Math.floor(coordXprocent - positionX);
			console.log(postsItemsWidth);
			positionX = positionX + (distX - speed);
			let position = postsDifferent / 200 * positionX ;
         
			postsItems.style.cssText = `transform: translate3d(${-position}px,0,0);`;

			if (Math.abs(distX) > 0) {
				requestAnimationFrame(setMouseGalleryStyle);
			} else {
				posts.classList.remove('_init');
			}

		}
		
	   posts.addEventListener("mousemove", function (e) {
			
			// Получение ширины 
			const postsWidth = posts.offsetWidth;

			//Ноль по середине
			const coordX = e.pageX - postsWidth / 2;
         
			//Получаем проценты 
			
			coordXprocent = coordX / postsWidth * 200;

			if (!posts.classList.contains('_init')){
				requestAnimationFrame(setMouseGalleryStyle);
				posts.classList.add('_init');
			}
		});
